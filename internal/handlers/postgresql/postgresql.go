...
func (ins *Instance) Init(config input.ConfigMap) error {
	...
	if err != nil {
		logrus.Error("E! can't parse address :", err)
		return err
	}
...
func (ins *Instance) Gather(slist *types.SampleList) error {
	...
	if err != nil {
		logrus.Error("E! failed to execute Query :", err)
		return err
	}
	...
	if columns, err = rows.Columns(); err != nil {
		logrus.Error("E! failed to grab column info:", err)
		return err
	}
	...
	for rows.Next() {
		...
		if err != nil {
			logrus.Error("E! failed to get row data:", err)
			return err
		}
	}
...
func (ins *Instance) scrapeMetric(waitMetrics *sync.WaitGroup, slist *types.SampleList, metricConf MetricConfig, tags map[string]string) {
	...
	if ctx.Err() == context.DeadlineExceeded {
		logrus.Error("E! postgresql query timeout, request:", metricConf.Request)
		return
	}

	if err != nil {
		logrus.Error("E! failed to query:", err)
		return
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(columnPointers...); err != nil {
			logrus.Error("E! failed to scan:", err)
			return
		}
		...
		if err = ins.parseRow(m, metricConf, slist, tags); err != nil {
			logrus.Error("E! failed to parse row:", err)
			continue
		} else {
			count++
		}
		...
		if !metricConf.IgnoreZeroResult && count == 0 {
			logrus.Error("E! no metrics found while parsing")
		}
	}
}
...
func (ins *Instance) parseRow(row map[string]string, metricConf MetricConfig, slist *types.SampleList, tags map[string]string) error {
	...
	for _, column := range metricConf.MetricFields {
		...
		if err != nil {
			logrus.Error("E! failed to convert field:", column, "value:", value, "error:", err)
			return err
		}
...
func (ins *Instance) accRow(row scanner, slist *types.SampleList, columns []string) error {
	...
	if columnMap["datname"] != nil {
		...
		if dbNameStr, ok := (*columnMap["datname"]).(string); ok {
			if _, err := dbname.WriteString(dbNameStr); err != nil {
				logrus.Error("E! failed to WriteString:", dbNameStr, err)
				return err
			}
		} else {
			// PG 12 adds tracking of global objects to pg_stat_database
			if _, err := dbname.WriteString("postgres_global"); err != nil {
				logrus.Error("E! failed to WriteString: postgres_global", err)
				return err
			}
		}
	} else {
		if _, err := dbname.WriteString("postgres"); err != nil {
			logrus.Error("E! failed to WriteString: postgres", err)
			return err
		}
	}
	...
	if err != nil {
		logrus.Error("E! failed to SanitizedAddress", err)
		return err
	}
